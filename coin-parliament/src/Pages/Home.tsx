...
const Home = () => {
  ...
  const {login, firstTimeLogin,setLogin,setLoginRedirectMessage} = useContext(AppContext);
  ...
  return (
    <>
      <div className="p-0 w-100" style={{background:'#160133'}}>
          ...
          <HomeContainer width={width} className="mb-4 p-0">
            ...
            {!(login || firstTimeLogin) && (<>
              <Image src={src} style={{
                ...
                height: width && width > 969 ? "auto" : ((width || 0) * 392 / 408),
                // marginTop: width && width > 969 ? -50 : -130,
                // marginTop:'120px',
                position: "absolute"
              }}/>
              <div className="w-100 " style={{marginTop:window.screen.width>979?'150px':'-48px'}}>
                <h2 style={{zIndex: 1, position: "relative"}} className="d-xl-none d-block">
                  <strong className="text-uppercase" style={{fontSize:'24px',fontWeight:'700'}}>{translate("Vote to Earn")}</strong>
        ...
        <TextContainer className="mt-2 d-xl-none" style={{textTransform:'none',fontWeight:'400', maxWidth:'250px'}}>
        <H2 style={{zIndex: 1, fontWeight:'400', position: "relative", marginTop:'200px'}}>{translate("Here's your chance to VOTE, IMPACT & EARN! ")}</H2>
                </TextContainer>
         {window.screen.width>979 && <H2  style={{zIndex: 1, fontWeight:'400', position: "relative", marginTop:'200px',fontSize:'30px'}}>{translate("Here's your chance to VOTE, IMPACT & EARN! ")}</H2>}
          <Coins
        ...
        <div className="mb-4 mx-0">
          <H2 style={{zIndex: 1, fontWeight:'400', position: "relative", marginBottom:'20px'}}>{translate("")}</H2>
          <Pairs
            onFavClick={async (...args) => {
              if (user) {
                await calcFavorites(...args);
        ...
        <div className="mb-5 mx-0" >
          <div className="mb-4" >
            <H2 style={{zIndex: 1, fontWeight:'400', position: "relative"}}>{translate("Influencers")}</H2>
          </div>
          <InfluencersCarousel/>
        </div>
        <div className="mb-5 mx-0">
          <div className="mb-4">
            <H2 style={{zIndex: 1, fontWeight:'400', position: "relative"}}>{translate("")}</H2>
          </div>
          <div>
