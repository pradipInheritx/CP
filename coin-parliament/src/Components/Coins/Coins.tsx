...
import {BearVsBullRow, getData, getFilteredData, getFilteredDataByFav} from "../../common/models/CoinTable";
...
const Container = styled.div`
  ...
  grid-template-columns:  ${(props: { width: number }) => props.width > 979 ? "373px 1fr 15px" : "15px 1fr 15px"};
  align-items: center;
  justify-content: center;
  width: 100%;
  max-width: ${(props: { width: number }) => props.width > 979 ? "759px" : "312px"};
`;
...
const Coins = ({
  ...
  const [data, setData] = useState<BearVsBullRow[]>(getData({coins, totals, allCoins}));
...
console.table(coins)
  ...
  return (
    <div className="d-flex flex-column justify-content-center align-items-center py-3">
      {expanded && (
        <Container  {...{width}} style={{margin:width > 979?'20px 0 50px 0':'"0 -15px'}}>
          {width > 979?
           <div style={{fontSize:'20px',fontWeight:'400',marginLeft:'20px'}}>What's Your Coin Vote ?</div>
          : <div/> }
         
          <div className="px-1" style={{position:'relative'}}>
          <i className="bi bi-search" style={{position:'absolute',top:'9px',right:'20px',fontSize:'18px'}}></i>
            <Input
              style={{
