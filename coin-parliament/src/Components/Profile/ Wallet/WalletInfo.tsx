...
function WalletInfo() {
    ...
    useEffect(() => {
        // @ts-ignore
        setWalletDetails({coin: userInfo?.wellDAddress?.coin || '',walletAddress: userInfo?.wellDAddress?.address || '',});
        // @ts-ignore
    ...
    const setDefaultValue = () => {
        // @ts-ignore
        setTimeAmount({amount: userInfo?.referalReceiveType?.amount || '',time: userInfo?.referalReceiveType?.time ? userInfo?.referalReceiveType?.time.replace(/[A-Z\s]/g, '') : ''});
        // @ts-ignore
