...
export const imageUploadFunction = async (req: any, res: any) => {
    ...
    try {
        const checkId = forModule.toUpperCase() === "CARD" ? await getCardDetails(id) : forModule.toUpperCase() === "ALBUM" ? await getAlbumDetails(id) : { albumName: "", cardName: "", message: true };
        if (checkId?.message) {
...
export const avatarUploadFunction = async (req: any, res: any) => {
    ...
    try {
        ...
        const newFileName = getUserName?.displayName + userId;
