...
export const getRewardNFT = async (req:any, res :any , next:any )=>{
  try {
    let { page = 1, limit = 5, orderBy = "albumName", sort = "desc"} = req.query;
    limit = parseInt(limit);


    let orderByConsolidate = "";
    ...
    switch(orderBy){
      ...
      case "cardType":
        orderByConsolidate = "setDetails.cardDetails.cardType";
        break

      case "cardName":
        orderByConsolidate = "setDetails.cardDetails.cardName";
        break

      case "status":
        orderByConsolidate = "setDetails.cardDetails.status";
        break
    ...
    let getAllRewardNFTs :any;

    getAllRewardNFTs = await firestore()
    .collection("nftGallery")
    ...
    const getAllRewardNFTsResponse = getAllRewardNFTs.docs.map((doc:any)=>({
      albumId : doc.id,
      albumName : doc.data()?.albumName ? doc.data()?.albumName : "",
      setDetails : doc.data()?.setDetails ? doc.data()?.setDetails : "",
      setQuantity : doc.data()?.setQuantity ? doc.data()?.setQuantity : "",
    })
    );

    const rewardNFTRef = await firestore().collection('nftGallery').get();
    let totalCount = rewardNFTRef.size;

    res.status(200).send({
      status: true,
      message: "Reward NFT albums fetched successfully",
      result: { data: getAllRewardNFTsResponse, totalCount },
    });
