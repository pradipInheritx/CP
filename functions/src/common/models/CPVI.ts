...
import { VoteProps, VoteResultProps } from "../types/Vote.type"
...
export const CPVIForCoin = async (coinName: string) => {
  try {
    ...
    const before24hoursTime = (currentTime - 1 * 1 * 3600000); //Update 24 hours to 7 days
    ...
    getCoinListing.forEach((coin: any) => {
      if (coin.direction == 0) countVoteObj.bull += 1
