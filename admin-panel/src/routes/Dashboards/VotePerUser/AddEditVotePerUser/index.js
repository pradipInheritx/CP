  ...
  useEffect(
    () => {
      if (currentVotePerUser) {        
        setCoinName(currentVotePerUser.coin);
        setPairName(currentVotePerUser.pair);      
        setQuantity(currentVotePerUser.quantity);      
      }
    },
    [ currentVotePerUser ]
  );
...
const onSubmitClick = () => {  
  ...
  const onUserSave = () => {
    ...
    if (currentVotePerUser) {
      dispatch(
        updateVotePerUser({...currentVotePerUser, ...timeFrameDetail}, () => {
          onCloseDialog();
        })
      );
    } else {
