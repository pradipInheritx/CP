...
import { deleteUser, getUsers, setCurrentUser } from '../../../redux/actions/UsersDetelis';
...
const UsersModule = () => {
  ...
  const handleSelectAllClick = event => {
    if (event.target.checked) {
      const newSelected = usersDetelisList.map(n => n.id);
        ...
        <TableContainer className={classes.container}>
          <Table stickyHeader className={classes.table} aria-labelledby="tableTitle" aria-label="sticky enhanced table">
            <UserTableHead
              ...
              rowCount={usersDetelisList.length}
            />
            <TableBody>
              {!!usersDetelisList.length ? (
                stableSort(usersDetelisList, getComparator(order, orderBy))
                ...
                <TableRow style={{ height: 53 * 6 }}>
                  <TableCell colSpan={7} rowSpan={10}>
                    {isFilterApplied ? (
                      ...
                      <NoRecordFound>{usersFetched ? 'There are no records found.' : 'Loading usersDetelisList...'}</NoRecordFound>
                    )}
                  </TableCell>
                </TableRow>
              )}
