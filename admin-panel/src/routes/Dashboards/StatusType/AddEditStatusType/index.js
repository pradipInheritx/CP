...
const AddEditStatusType = ({open, onCloseDialog}) => {
  ...
  useEffect(
    () => {
      if (currentStatusType) {        
        setTypeName(currentStatusType.typeName);
        setPrice(currentStatusType.price);      
        setVotes(currentStatusType.votes);      
      }
    },
    [ currentStatusType ]
  );
...
const onSubmitClick = () => {  
  ...
  const onUserSave = () => {
    ...
    if (currentStatusType) {
      dispatch(
        updateVotePerUser({...currentStatusType, ...Detail}, () => {
          onCloseDialog();
        })
      );
    } else {
  ...
  return (
    <Dialog open={open} onClose={onCloseDialog} className={classes.dialogRoot}>
      <DialogTitle className={classes.dialogTitleRoot}>
        {currentStatusType ? "Edit Status Type" : "Create Status Type"}
      </DialogTitle>
