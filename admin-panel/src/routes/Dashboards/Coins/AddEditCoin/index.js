...
import {addNewCoin, updateCoin} from "../../../../redux/actions/Coins";
...
const AddEditCoin = ({open, onCloseDialog}) => {
  ...
  const onUserSave = phoneNumbers => {
    ...
    if (currentUser) {
      dispatch(
        updateCoin({...currentUser, ...userDetail}, () => {
          onCloseDialog();
        })
      );
    } else {
      dispatch(
        addNewCoin(userDetail, () => {
          onCloseDialog();
        })
      );
    }
