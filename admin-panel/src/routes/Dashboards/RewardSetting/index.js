...
const RewardSettingModule = () => {
  ...
  useEffect(() => {
    ...
    RewardSettingDetelis &&
      [RewardSettingDetelis].map((item, index) => {
        CmpValue = Object.keys(item);
      });

    CmpValue &&
      CmpValue.map((item, index) => {
  ...
  const onCmpRewardSubmit = finalRewardInfo => {
    dispatch(updateRewardSetting(finalRewardInfo));
  };
  ...
  const AddRemoveInfo = (type, index) => {
    ...
    if (type == "add") {
      ...
      setCmpNumber([
        ...CmpNumber,
        Number(CmpNumber[CmpNumber.length - 1]) + 100
      ]);
    }
    if (type == "remove") {
      const forReward = allRewardInfo.filter(
        (item, itemindex) => itemindex !== index
      );
      const forCmpNum = allCmpNumber.filter(
        (item, itemindex) => itemindex !== index
      );
  ...
  const handelInputText = (e, index, childIndex, type) => {
    const allRewardInfo = [...rewardInfo];
    allRewardInfo[index][`${type}`][childIndex] = e.target.value.replace(
      /[^0-9]/g,
      ""
    );
  ...
  return (
    <div className={classes.root}>
      <Paper className={classes.paper} style={{ padding: "30px" }}>
        <h3
          style={{ marginBottom: "5px", fontWeight: "bold", fontSize: "24px" }}
        >
          Reward Setting
        </h3>
          ...
          <Box className="" style={{ marginTop: "10px" }}>
            {rewardInfo?.map((item, index) => {
              return (
                <>
                  <Box>
                    <p htmlFor="" style={{ margin: "10px" }}>
                      Cmp For {CmpNumber[index]} :
                    </p>
                    <Box
                      style={{
                        display: "flex",
                        ...
                        flexWrap: "wrap",
                        border: `${
                          rewardInfoError[index] != undefined &&
                          rewardInfoError[index] != ""
                            ? "1px solid red"
                            : ""
                        }`
                      }}
                    >
                      {item?.cardTierPickingChanceInPercent?.map(
                        (cardTierItem, cardIndex) => {
                          return (
                            <>
                              <TextField
                                style={{ width: "10%" }}
                                ...
                                onChange={event => {
                                  ...
                                  handelInputText(
                                    ...
                                    index,
                                    cardIndex,
                                    "cardTierPickingChanceInPercent"
                                  );
                                  setRewardInfoError([]);
                                }}
                                // defaultValue={0}
                      ...
                      {item?.extraVotePickFromRange?.map(
                        (GameItem, GameIndex) => {
                          return (
                            <>
                              <TextField
                      ...
                      {item?.diamondsPickFromRange?.map(
                        (BoostItem, BoostIndex) => {
                          return (
                            <>
                              <TextField
                                style={{ width: "10%" }}
                  ...
                  {rewardInfo.length - 1 == index && (
                    <Box>
                      ...
                      {rewardInfo.length - 1 > 0 && (
                        <Button
                          onClick={() => {
                            AddRemoveInfo("remove", index);
                          }}
                        >
                          Remove
                        </Button>
