...
export const addNewVoteBoost = (timeFrameDetail, callbackFun) => {
  return dispatch => {
    ...
    dispatch(fetchStart());
    axios
      .post(endpoint.voteCreateTimeframe, { ...timeFrameDetail })
      .then(data => {
...
export const updateVoteBoost = (timeFrame, callbackFun) => {
  return dispatch => {
    ...
    axios
      .put(
        `${endpoint.voteUpdateTimeframe}/${timeFrame.timeframeId}`,
        timeFrame
      )
      .then(response => {
        if (
...
export const updateVoteBoostStatus = (id, data, callbackFun) => {
  ...
  return dispatch => {
    ...
    dispatch(fetchStart());
    axios
      .put(`${endpoint.voteUpdateTimeframe}/${id}`, data)
      .then(response => {
        if (
          response.status === 200 ||
          response.status === 201 ||
...
export const deleteBulkVoteBoost = (userIds, callbackFun) => {
  return dispatch => {
    dispatch(fetchStart());
    axios
      .put(endpoint.voteBulkDelete, { userIds })
      .then(response => {
...
export const deleteVoteBoost = (userId, callbackFun) => {
  return dispatch => {
    dispatch(fetchStart());
    axios
      .delete(`${endpoint.voteDeleteTimeframeById}/${userId}`)
      .then(data => {
        if (data.status === 200) {
