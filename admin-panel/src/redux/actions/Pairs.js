...
export const addNewPair = (pair, callbackFun) => {
  return dispatch => {
    dispatch(fetchStart());
    axios
      .post(endpoint.createPairCoin, pair)
      .then(data => {
...
export const updatePair = (pair, VoteBarUpdate, callbackFun) => {
  return dispatch => {
    dispatch(fetchStart());
    axios
      .patch(`${endpoint.voteBarRange}/${pair.id}`, VoteBarUpdate)
      .then(response => {
...
export const updatePairStatus = (id, data, callbackFun) => {
  ...
  return dispatch => {
    dispatch(fetchStart());
    axios
      .patch(`${endpoint.updateCoinPairStatus}/${id}`, data)
      .then(response => {
...
export const deleteBulkPair = (userIds, callbackFun) => {
  return dispatch => {
    dispatch(fetchStart());
    axios
      .put(endpoint.usersBulkDelete, { userIds })
      .then(response => {
        if (response.status === 200) {
...
export const deletePair = (userId, callbackFun) => {
  return dispatch => {
    dispatch(fetchStart());
    axios
      .delete(`${endpoint.usersDeleteSubAdmin}/${userId}`)
      .then(data => {
        if (data.status === 200) {
