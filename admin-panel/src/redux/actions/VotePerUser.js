...
export const getVotePerUser = (
  ...
  return dispatch => {
    ...
    axios
      .get(
        `${endpoint.voteGetPerUserVote}?limit=${rowsPerPage}&page=${page +
...
export const addNewVotePerUser = (timeFrameDetail, callbackFun) => {
  return dispatch => {
    ...
    dispatch(fetchStart());
    axios
      .post(endpoint.voteCreatePerUserVote, { ...timeFrameDetail })
      .then(data => {
...
export const updateVotePerUser = (timeFrame, callbackFun) => {
  return dispatch => {
    ...
    axios
      .put(
        `${endpoint.voteUpdatePerUserVoteById}/${timeFrame.perUserVoteId}`,
        timeFrame
      )
      .then(response => {
        if (
...
export const updateVotePerUserStatus = (id, data, callbackFun) => {
  ...
  return dispatch => {
    ...
    dispatch(fetchStart());
    axios
      .put(`${endpoint.votePassUpdateTimeframe}/${id}`, data)
      .then(response => {
        if (
          response.status === 200 ||
          response.status === 201 ||
...
export const deleteVotePerUser = (userId, callbackFun) => {
  return dispatch => {
    dispatch(fetchStart());
    axios
      .delete(`${endpoint.votePassDeletePerUserVoteById}/${userId}`)
      .then(data => {
        if (data.status === 200) {
