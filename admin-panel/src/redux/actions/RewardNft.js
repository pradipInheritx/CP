      ...
      .then(data => {
        if (data.status === 200 || data.status === 201 || data.status === 204) {
          ...
          if (data.data.result.albumId && videoUrl) {
            axios
              .post(
                `${ForVideoImg}ALBUM/video/${data.data.result.albumId}`,
                formData,
                config
              )
              .then(data => {
                dispatch({
                  type: EDIT_REWARDALBUMIMG,
...
export const updateRewardAlbum = (
  Albumid,
  AlbumData,
  videoUrl,
  callbackFun
    ...
    axios
      ...
      .then(data => {
        if (data.status === 200 || data.status === 201 || data.status === 204) {
          ...
          if (Albumid && videoUrl) {
            axios
              .post(`${ForVideoImg}ALBUM/video/${Albumid}`, formData, config)
              .then(data => {
                dispatch({
                  type: EDIT_REWARDALBUMIMG,
                  payload: data.data.result
      ...
      .then(data => {
        if (data.status === 200 || data.status === 201 || data.status === 204) {
          ...
          if (data.data.result.cardId && cardImage) {
            axios
              .post(
                `${ForVideoImg}CARD/image/${data.data.result.cardId}`,
                formData,
                config
              )
              .then(imgdata => {
                dispatch({
                  type: EDIT_REWARDCARD,
                  payload: imgdata.data.result
                });
      ...
      .then(data => {
        if (data.status === 200 || data.status === 201 || data.status === 204) {
          ...
          if (cardId && cardImageUrl) {
            ...
            axios
              .post(`${ForVideoImg}CARD/image/${cardId}`, formData, config)
              .then(imgdata => {
                dispatch({
                  type: EDIT_REWARDCARD,
                  payload: imgdata.data.result
