...
export const addNewStatusType = (timeFrameDetail, callbackFun) => {
  return dispatch => {
    ...
    dispatch(fetchStart());
    axios
      .post(endpoint.createTimeframe, { ...timeFrameDetail })
      .then(data => {
...
export const updateStatusType = (timeFrame, callbackFun) => {
  return dispatch => {
    ...
    axios
      .put(
        `${endpoint.statusUpdateTimeframe}/${timeFrame.timeframeId}`,
        timeFrame
      )
      .then(response => {
        if (
...
export const updateStatusTypeStatus = (id, data, callbackFun) => {
  ...
  return dispatch => {
    ...
    dispatch(fetchStart());
    axios
      .put(`${endpoint.statusUpdateTimeframe}/${id}`, data)
      .then(response => {
        if (
          response.status === 200 ||
          response.status === 201 ||
...
export const deleteBulkStatusType = (userIds, callbackFun) => {
  return dispatch => {
    dispatch(fetchStart());
    axios
      .put(endpoint.statusBulkDelete, { userIds })
      .then(response => {
...
export const deleteStatusType = (userId, callbackFun) => {
  return dispatch => {
    dispatch(fetchStart());
    axios
      .delete(`${endpoint.deleteTimeframeById}/${userId}`)
      .then(data => {
        if (data.status === 200) {
