...
export const addNewTrTypes = (timeFrameDetail, callbackFun) => {
  return dispatch => {
    ...
    dispatch(fetchStart());
    axios
      .post(endpoint.createTimeframeTr, { ...timeFrameDetail })
      .then(data => {
...
export const updateTrTypes = (timeFrame, callbackFun) => {
  return dispatch => {
    ...
    dispatch(fetchStart());
    axios
      .put(`${endpoint.updateTimeframeTr}/${timeFrame.timeframeId}`, timeFrame)
      .then(response => {
        if (
          response.status === 200 ||
          response.status === 204 ||
...
export const deleteBulkTrTypes = (userIds, callbackFun) => {
  return dispatch => {
    dispatch(fetchStart());
    axios
      .put(endpoint.bulkDeleteTr, { userIds })
      .then(response => {
...
export const deleteTrTypes = (userId, callbackFun) => {
  return dispatch => {
    dispatch(fetchStart());
    axios
      .delete(`${endpoint.deleteTimeframeByIdTr}/${userId}`)
      .then(data => {
        if (data.status === 200) {
